...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.status === "success") {
        ...
        setErrorMsg(res.data.message || "Invalid credentials provided.");
      }
    } catch (err) {
      setIsLoading(false);
      console.error("Login Error:", err);
      setErrorMsg(err.response?.data?.message || "Network error. Please try connecting again.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans selection:bg-accent selection:text-gray-900">
      ...
      {/* Background Shapes */}
      <div className="absolute top-0 left-0 -translate-y-12 -translate-x-1/3 z-0 pointer-events-none">
        <div className="w-[600px] h-[600px] rounded-full bg-brand-200/40 blur-3xl opacity-60"></div>
      </div>
        ...
        <div className="max-w-md w-full animate-slide-up">
          ...
          <div className="glass-card p-10 mt-12 rounded-3xl">
            <div className="text-center mb-10">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-accent/20 text-accent-dark mb-6 shadow-sm">
              ...
              <p className="mt-3 text-sm text-gray-500 font-medium">
                Log in to access your dashboard and book rides.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="pt-2">
                ...
                >
                  {isLoading ? (
                    <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-gray-900" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                  ) : "Log In"}
                </button>
              </div>
            </form>
